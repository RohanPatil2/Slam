...
class ECSLogger(Node):
    ...
    def generate_plots(self, base_filename):
        ...
        try:
            ...
            # Plot 1: Average map entropy over time
            if self.avg_map_entropy_data:
                times, entropies = zip(*self.avg_map_entropy_data)
                axes[0, 0].plot(times, entropies, 'g-', linewidth=2)
                axes[0, 0].set_xlabel('Time (s)')
                axes[0, 0].set_ylabel('Avg Map Entropy (normalized)')
                axes[0, 0].set_title('Average Map Entropy Evolution')
                axes[0, 0].grid(True, alpha=0.3)

            # Plot 2: Max map entropy over time
            if self.max_map_entropy_data:
                times, entropies = zip(*self.max_map_entropy_data)
                axes[0, 1].plot(times, entropies, 'r-', linewidth=2)
                axes[0, 1].set_xlabel('Time (s)')
                axes[0, 1].set_ylabel('Max Map Entropy (normalized)')
                axes[0, 1].set_title('Maximum Map Entropy Evolution')
                axes[0, 1].grid(True, alpha=0.3)

            # Plot 3: Map exploration progress
            if self.map_size_data:
                times, total_cells, known_cells = zip(*self.map_size_data)
                exploration_ratio = [k/t*100 if t > 0 else 0 for k, t in zip(known_cells, total_cells)]
                axes[1, 0].plot(times, exploration_ratio, 'm-', linewidth=2)
                axes[1, 0].set_xlabel('Time (s)')
                axes[1, 0].set_ylabel('Exploration (%)')
                axes[1, 0].set_title('Map Exploration Progress')
                axes[1, 0].grid(True, alpha=0.3)

            # Plot 4: Both entropy metrics together
            axes[1, 1].set_xlabel('Time (s)')
            axes[1, 1].set_ylabel('Entropy (normalized)')
            axes[1, 1].set_title('Entropy Metrics Comparison')
            if self.avg_map_entropy_data:
                times, entropies = zip(*self.avg_map_entropy_data)
                axes[1, 1].plot(times, entropies, 'g-', linewidth=2, label='Average')
            if self.max_map_entropy_data:
                times, entropies = zip(*self.max_map_entropy_data)
                axes[1, 1].plot(times, entropies, 'r-', linewidth=2, label='Maximum')
            axes[1, 1].legend()
    ...
    def auto_save_callback(self):
        """Periodic auto-save."""
        if len(self.avg_map_entropy_data) > 0 or len(self.max_map_entropy_data) > 0:
